...
| User | Interaction |
|---|---|
| Field worker | Sends Telegram voice/image/text — nothing else needed |
| NGO coordinator | Views live crisis map, oversees autonomous dispatch |
| Volunteer | Receives Telegram task card with role, route, ETA |
...
Ushahidi maps crises manually. Zelos coordinates after tasks are entered. Nobody has connected unstructured Telegram chaos → autonomous dispatch. That gap only became buildable with Gemini multimodal in 2024–25.
